...
async def update(ctx):
    ...
    try:
        os._exit(0)  # ใช้วิธีปิดบอทให้โฮสต์รันใหม่เอง
...
async def mute_channel(ctx):
    ...
    try:
        ...
        await ctx.send(f"🔇 ปิดแจ้งเตือนและ @mention สำหรับช่องที่มี ID {channel_id} เรียบร้อยแล้ว")
        await log_message(bot, f"🔇 ปิดแจ้งเตือนและ @mention สำหรับช่องที่มี ID {channel_id} โดย {ctx.author} ({ctx.author.id})")
...
async def delete(ctx, target: discord.Member = None, number: int = 0):
    ...
    if target is not None:
        ...
        await ctx.send(f"🗑️ ลบข้อความทั้งหมด {len(deleted)} ข้อความจาก {target.display_name} ({target.id}) เรียบร้อยแล้ว")
        await log_message(bot, f"🗑️ {ctx.author} ({ctx.author.id}) ลบข้อความทั้งหมด {len(deleted)} ข้อความจาก {target.display_name} ({target.id})")
    elif number > 0:
        ...
        await ctx.send("❌ ระบุจำนวนข้อความที่ต้องการลบหรือผู้ใช้ที่ต้องการลบข้อความจาก")
